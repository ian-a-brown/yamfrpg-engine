...
class DeviseCreateYamfrpgEngineUsers < ActiveRecord::Migration[7.1]
  ...
  def change
    ...
    add_index :yamfrpg_engine_users, :email,                unique: true
    add_index :yamfrpg_engine_users, :reset_password_token, unique: true
    # add_index :yamfrpg_engine_users, :confirmation_token,   unique: true
    add_index :yamfrpg_engine_users, :unlock_token,         unique: true
  end
end
